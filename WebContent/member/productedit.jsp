...
<section class="layui-larry-box">
	<div class="larry-personal">
		...
		<div class="larry-personal-body clearfix changepwd">
			<form class="layui-form col-lg-4" id="supplierform" method="post" action="<%=path %>/member/productEdit.do" onsubmit="return supplieradd()">
			<input type="hidden" name="id" value="${product.id}"/>
				<div class="layui-form-item">
					<label class="layui-form-label">商品名称</label>
					<div class="layui-input-block">  
					  	<input type="text" name="productname" id="productname" value="${product.productname}" required oninvalid="setCustomValidity('商品名称不能为空')" oninput="setCustomValidity('');"  style="width: 300px;"  autocomplete="off"  class="layui-input" placeholder="商品名称" >
					</div>
				</div>
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block" style="width: 300px;">  
					  	<select name="cid" id="cid" style="width: 300px;"   lay-verify="required" lay-filter="cid"  autocomplete="off"  class="layui-input">
					  	  <option value="${prodcut.cid}">${product.ccategory.name}</option>
					  	</select>
					</div>
				</div>
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">  
					  	<input type="text" name="oldprice" id="oldprice" lay-verify="required" value="${product.oldprice}"  required oninvalid="setCustomValidity(原价不能为空或价格输入有误')" oninput="setCustomValidity('');"  pattern="(0|[1-9][0-9]{0,9})(\.[0-9]{1,2})?" style="width: 300px;"  autocomplete="off"  class="layui-input" placeholder="商品单价" >
					</div>
				</div>
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">  
					  	<input type="text" name="price" id="price"  lay-verify="required" value="${product.price}" onblur="compare()" required oninvalid="setCustomValidity('现价不能为空或价格输入有误')" oninput="setCustomValidity('');"  pattern="(0|[1-9][0-9]{0,9})(\.[0-9]{1,2})?" style="width: 300px;"  autocomplete="off"  class="layui-input" placeholder="商品单价" >
					</div>
				</div>
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">  
					  	<textarea id="editor_id" name="content" style="width: 600;height: 280px;" placeholder="请输入商品介绍" required class="layui-textarea" oninvalid="setCustomValidity('商品介绍不能为空')" oninput="setCustomValidity('');" >${product.content}</textarea>
					</div>
				</div>
...
<script type="text/javascript">
	...
	layui.use(['form','upload'],function(){
        ...
        form.on('select(fid)',function(){
			var fid = $("#fid").val();
			$.ajax({
	        	  url:"../admin/searchCtype.do?fid="+fid,
	        	  datetype:"json",
	        	  type:"post",
        ...
        layui.use(['form','upload'],function(){
        	var form = layui.form();
        	 form.on('select(cid)',function(){
     			var cid = $("#cid").val();
     			$.ajax({
     	        	  url:"../admin/searchCtype.do?fid="+cid,
     	        	  datetype:"json",
     	        	  type:"post",
     	        	  contentType: "application/Json; charset=UTF-8",
     	        	  success:function(msg){
