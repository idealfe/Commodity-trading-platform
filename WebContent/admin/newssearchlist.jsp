	...
	<div class="larry-personal">
		...
		<div class="layui-inline">
			<div class="layui-input-inline">
		      <form action="<%=path %>/admin/searchNews.do" method="post" id="searchform" name="searchform">
                ...
                <div class="layui-form news_list" >
					...
					<tbody class="news_content" id="userTbody">
					   <c:forEach items="${list}" var="news" varStatus="status">
						<tr >
						    <td>
						    <input name="checked" lay-skin="primary" lay-filter="choose" type="checkbox" value="${news.id}">
						    <div class="layui-unselect layui-form-checkbox" lay-skin="primary"><i class="layui-icon"></i></div>
						    </td>
							<td>${news.title}</td>
							<td>${news.savetime}</td>
							<td>
							    <a class="layui-btn layui-btn-mini" href="showNews.do?id=${news.id}"><i class="iconfont icon-edit"></i> 编辑</a>
							</td>
						</tr>
						</c:forEach>
						<tr>
                    <td align="center" style="font-weight: bold;font-family:楷体;font-weight: bold; color:blue" colspan="10">
                                                                 共${total}条记录&nbsp;&nbsp;&nbsp;&nbsp;
                        <a href="searchNews.do?index=1&key=${key}" style="font-family:楷体;">首页</a>&nbsp;&nbsp;
                        
                        <c:choose>
                        <c:when test="${index >1}">
                        <a href="searchNews.do?index=${index-1}&key=${key}" style="font-family:楷体;">上一页</a>
                        </c:when>
                        <c:otherwise>
                        ...
                        <a href="searchNews.do?index=${index+1}&key=${key}" style="font-family:楷体;">下一页</a>
                        </c:when>
                        <c:otherwise>
                        ...
                        &nbsp;&nbsp;
                        <a href="searchNews.do?index=${pages}&key=${key}" style="font-family:楷体;">末页</a>
                    </td>
                  </tr>
					</tbody>
					</table>
...
<script type="text/javascript">
    ...
    function deleteAll(){
    	...
    	layer.confirm('是否确认删除？', {
            ...
            $.ajax({
      		type:"post",
      		url:"newsDelAll.do?vals="+vals,
      		date:"",
      		success:function(msg){
      			location.replace("newsList.do");
      		  }
            })
      	  }else{
				layer.msg("请选择要删除的项");
