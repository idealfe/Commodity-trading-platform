...
%>
			<form class="layui-form col-lg-4" id="categoryform" method="post" action="<%=path %>/admin/categoryAdd.do" onsubmit="return categoryadd()">
			<input type="hidden" value="<%=fatherid %>" name="fatherid">
			<input type="hidden" value="<%=type %>" name="type">
				<div class="layui-form-item">
					<label class="layui-form-label">商品类别名称</label>
					<div class="layui-input-block">  
					  	<input type="text" name="name" id="name" required oninvalid="setCustomValidity('商品类别名称不能为空')" oninput="setCustomValidity('');"  style="width: 300px;"  autocomplete="off"  class="layui-input" placeholder="商品类别名称" >
					</div>
				</div>
